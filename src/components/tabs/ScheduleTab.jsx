...
export default function ScheduleTab() {
  ...
  return (
    <div className="glass-panel rounded-2xl p-6">
      <h2 className="text-lg font-semibold text-emerald-600 mb-6 flex items-center gap-2">
        <Clock size={20} /> Daily Timeline
      </h2>
      
      <div className="relative border-l border-gray-200 ml-3 space-y-8 pb-4">
        {sortedTimes.length === 0 && (
          <div className="pl-6 text-sm text-gray-400">No scheduled items found. Add schedule times to your supplements and meals!</div>
        )}
        ...
        {sortedTimes.map((time, idx) => {
          ...
          return (
            <div key={`${time}-${idx}`} className="relative pl-6">
              <div className={`absolute -left-[1.1rem] top-1 rounded-full border-4 border-[#0b0f19] bg-gray-100 p-1 ${colorClass}`}>
                <Icon size={14} />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {items.map(item => (
                  <div key={`${item.type}-${item.id}`} className="bg-white shadow-sm border border-gray-100/50 border border-gray-200 p-4 rounded-xl transition-all hover:bg-gray-100/80">
                    ...
                    <p className="text-[11px] text-gray-400 mt-2">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
