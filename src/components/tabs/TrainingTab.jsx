    ...
    <div className="glass-panel rounded-2xl p-4 md:p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Microcycle Focus: 6:30 AM Structural Adaptation & Joint Sparing</p>
        </div>
        <div className="w-full sm:w-auto">
          <select 
            value={selectedDayId}
            onChange={(e) => setSelectedDayId(e.target.value)}
            className="w-full bg-white shadow-sm border border-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 font-medium focus:outline-none focus:border-amber-500"
          >
            {workouts.map(w => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {!targetBlock || targetBlock.exercises.length === 0 ? (
          <div className="col-span-full py-6 text-center text-gray-400 text-sm">No exercise data mapped to this session profile.</div>
        ) : (
          targetBlock.exercises.map((ex) => {
            const isDone = ex.isCompleted;
            return (
              <div key={ex.id} className={`relative flex flex-col p-4 rounded-xl border transition-all duration-300 ${isDone ? 'bg-emerald-950/20 border-emerald-900/50 opacity-75' : 'bg-white shadow-sm border border-gray-100/50 border-gray-200'}`}>
                ...
                <div className="flex justify-between items-start mb-3 gap-2">
                  <div className="flex-1">
                    <input 
                      ...
                      className={`font-semibold text-base bg-transparent border-none p-0 outline-none w-full ${isDone ? 'text-emerald-300' : 'text-gray-800'}`}
                    />
                    <div className="text-[11px] text-gray-400 mt-1 leading-relaxed">{ex.tip}</div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div className="flex flex-col bg-white shadow-sm border border-gray-100/50 rounded-lg p-2 border border-gray-200/50">
                    <span className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 font-semibold">Sets & Reps</span>
                    <div className="flex items-center">
                      <input 
                        type="number"
                        value={ex.sets} 
                        onChange={(e) => updateExercise(selectedDayId, ex.id, { sets: Number(e.target.value) })}
                        className="w-8 bg-transparent text-center text-sm text-gray-900 outline-none"
                      /> 
                  ...
                  <div className="flex flex-col bg-white shadow-sm border border-gray-100/50 rounded-lg p-2 border border-gray-200/50">
                    <span className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 font-semibold">Weight (kg)</span>
                    <input 
                      type="number" 
                      value={ex.weight} 
                  ...
                  <div className="flex flex-col bg-white shadow-sm border border-gray-100/50 rounded-lg p-2 border border-gray-200/50">
                    <span className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 font-semibold">RPE</span>
                    <input 
                ...
                <div className="mb-4">
                  <textarea 
                    ...
                    className="w-full bg-white shadow-sm border border-gray-100/50 border border-gray-200/50 rounded-lg p-2.5 text-xs text-gray-700 outline-none h-14 resize-none custom-scrollbar focus:border-amber-500/50"
                  />
                </div>
