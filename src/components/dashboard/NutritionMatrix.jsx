    ...
    <div className="glass-panel rounded-2xl p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Target Metrics: ~2,300 kcal | 200g Protein | 220g Carbs | 70g Fat</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {meals.map(meal => (
          <div key={meal.id} className="p-4 rounded-xl bg-white shadow-sm border border-gray-100/40 border border-gray-200 flex flex-col justify-between gap-3">
            <div>
              <div className="flex justify-between items-start">
                ...
                />
                <button onClick={() => deleteMeal(meal.id)} className="text-gray-400 hover:text-red-400 transition-colors">
                  <X size={14} />
                </button>
              </div>
              <input 
                ...
                className="bg-transparent font-medium text-sm text-gray-800 mt-2 border-none p-0 focus:ring-0 outline-none w-full"
              />
              <textarea 
                value={meal.desc}
                onChange={(e) => updateMeal(meal.id, { desc: e.target.value })}
                className="bg-transparent text-xs text-gray-500 mt-1.5 border-none p-0 focus:ring-0 outline-none w-full h-16 resize-none custom-scrollbar"
              />
            </div>
          </div>
        ))}
